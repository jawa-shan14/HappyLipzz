...
export const ServiceDetailModal: React.FC<ServiceDetailModalProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="bg-[#007D88] text-white px-6 sm:px-8 py-6 flex items-start justify-between">
        ...
        <div className="p-6 sm:p-8 space-y-6">
          <div>
            <h4 className="text-xs font-bold uppercase tracking-wider text-[#007D88] mb-2">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-2.5">
                {service.benefits.map((benefit, idx) => (
                  <div key={idx} className="flex items-start gap-2.5 text-sm text-[#0A2533]">
                    <CheckCircle className="w-4 h-4 text-[#007D88] shrink-0 mt-0.5" />
                    <span>{benefit}</span>
                  </div>
                ))}
              </div>
          ...
          {/* Quick Details */}
          <div className="bg-[#F4F9FA] rounded-2xl p-4 flex items-center justify-between border border-slate-100 text-xs">
            <div className="flex items-center gap-2 text-slate-700">
              <Clock className="w-4 h-4 text-[#007D88]" />
              <span>Est. Duration: <strong className="text-[#0A2533]">{service.duration || '30-45 mins'}</strong></span>
            </div>
            <div className="flex items-center gap-2 text-slate-700">
              <ShieldCheck className="w-4 h-4 text-[#007D88]" />
          ...
          <div className="pt-2 flex flex-col sm:flex-row gap-3">
            <button
              ...
              className="flex-1 py-3 px-5 rounded-full bg-[#007D88] hover:bg-[#006B74] text-white font-semibold text-sm transition-all shadow-xs flex items-center justify-center gap-2 cursor-pointer"
            >
            ...
            <a
              ...
              className="py-3 px-5 rounded-full border border-slate-200 hover:border-[#007D88] text-[#0A2533] font-medium text-sm transition-colors text-center"
            >
              Call Clinic
            </a>
          </div>
